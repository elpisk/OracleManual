...
2. **조절 변수는 파일 상단 '기본값 처리' 블록에 모여 있다.** 반복 횟수·대상 건수·세션 역할·정리 여부는
   스크립트를 부르기 전에 `DEFINE 변수 = 값` 으로 지정하고, 지정하지 않으면 기본값이 쓰인다.
   기본값은 일반적인 실습 VM에서 30초~2분(WL_06만 3~5분)에 끝나도록 잡았고, 그 산정 근거를 주석에 적어 두었다.
...
조절 변수는 스크립트를 부르기 전에 SQL\*Plus에서 `DEFINE wl_xxx = 값` 으로 지정한다(**지정한 값이 이긴다**).
지정하지 않은 변수만 스크립트 안의 기본값이 쓰인다 — SQL\*Plus 에 '미정의면 정의' 문법이 없어
`COLUMN ... NEW_VALUE` 관용구로 구현했다. 한 번 `DEFINE` 한 값은 세션이 끝날 때까지 남으므로,
다른 값으로 다시 돌리려면 `DEFINE` 을 다시 하거나 `UNDEFINE wl_xxx` 로 기본값으로 되돌린다.
