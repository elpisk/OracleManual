...
|---|---:|---:|---|
| `hosp_id=1`(스큐) | 2,795 | 3,058 | 근접(비율 약 0.91배) |
| `hosp_id=500`(일반) | 280 | 177 | 근접(비율 약 1.58배) |
...
| 조건 | E-Rows | A-Rows | 추정 정확도 |
|---|---:|---:|---|
| `hosp_id=1`(스큐, 복원 후) | 2,958 | 3,058 | 근접(비율 약 0.97배) |

- 히스토그램을 복원하자 다시 스큐를 반영한 정확한 추정으로 돌아옴(수치가 04절의
  2,795와 완전히 같지는 않은데, 이는 통계 수집이 표본추출 기반이라 매번 약간씩 다를 수
  있기 때문 — 그래도 두 값 다 실제값(3,058)의 ±10% 안에 있음은 동일)
- `METHOD_OPT => 'FOR ALL COLUMNS SIZE AUTO'`는 Oracle이 각 컬럼의 데이터 분포를 보고
  히스토그램이 필요한지 스스로 판단하게 하는 옵션 — 이 스키마처럼 스큐가 실제로 있는
  컬럼에는 자동으로 히스토그램을 만들어준다. 단, AUTO 는 WHERE 에 쓰인 적이 있는 다른 컬럼(CLAIM_ID·
  CLAIM_TYPE·TOTAL_AMT·RECEIPT_DATE 등)에도 히스토그램을 만들어 이후 실습의 E-Rows 와 계획을 바꾼다 —
  이 과정은 실습 03 [3] 에서 기준 상태(히스토그램 HOSP_ID 하나)로 되돌린다(과정구성 6장)

## 08. Benchmark

| 상태 | hosp_id=1 E-Rows | hosp_id=500 E-Rows | 두 값이 다른가? |
|---|---:|---:|---|
| 히스토그램 있음(원본) | 2,795 | 280 | 다름(스큐 반영) |
| 히스토그램 제거(SIZE 1) | 300 | 300 | **동일**(균등 가정) |
| 히스토그램 복원(AUTO) | 2,958 | 158 | 다시 다름 |
