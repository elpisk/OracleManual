...
## 6-2. 별도 산출물 — Recovery Catalog 고급 실습 (이수자 대상 심화)

19개 챕터와 종합 실습 과제를 이수한 사람을 대상으로 하는 심화 과정이다.
기본 과정이 DB 한 대를 혼자 관리하는 관점이라면, 이 과정은 여러 대를 카탈로그 하나로 통제하는 운영자 관점으로 올라간다.

**폴더**: `고급실습_RecoveryCatalog/`  · **환경**: 2대 서버(oel7v9r1 운영계 + oel7v9r2 관리계), 인스턴스 4개(orcl·sales / rcat·hrdb)

| # | 예제 | 소요 | 카탈로그 핵심 기능 |
|---|---|---|---|
| 01 | Virtual Private Catalog와 DBA 권한 분리 | 2.5h | `CREATE VIRTUAL CATALOG`, `GRANT CATALOG FOR DATABASE` |
| 02 | 다중 DB 중앙 등록과 백업 현황 리포팅 | 3.0h | `RC_*` 뷰 기반 판정 리포트 |
| 03 | 파라미터화 글로벌 Stored Script 백업 프레임워크 | 3.0h | `CREATE GLOBAL SCRIPT` + 치환 변수 |
| 04 | 카탈로그·컨트롤파일 불일치 해소와 공간 회수 | 2.5h | `CROSSCHECK`, `CATALOG START WITH` |
| 05 | 카탈로그 자체의 보호·통합·업그레이드 | 3.0h | `IMPORT CATALOG`, `UPGRADE CATALOG` |
| 06 | KEEP FOREVER 장기 보관과 보존 요건 대응 | 2.5h | `KEEP FOREVER` (카탈로그 필수) |
| 07 | 재해복구 — 카탈로그만으로 타 서버 전체 재구축 | 3.5h | 카탈로그에서 DBID·구조·손실 범위 확보 |
| 08 | 카탈로그 이력 분석과 백업 창 준수 튜닝 | 3.0h | 이력 추세 분석, BCT·SECTION SIZE |
| 09 | 암호화 백업의 외부 반출과 복원 | 2.5h | 이중 모드 암호화 + `KEEP` |
| 10 | 복구 가능성 상시 검증과 DR 훈련 런북 | 3.0h | 검증 4단계, RTO/RPO 실측 산정 |

총 28.5시간 / 5일 구성 권장. 예제는 순서대로 진행해야 하며 뒤 예제가 앞 예제의 산출물을 전제로 한다.

**산출물**

| 구분 | 파일 |
|---|---|
| 트랜스크립트 | `고급실습_01~10_*.txt` 10개 (예제당 805~1,200줄, 7단계 템플릿) |
| 과정 안내 | `고급실습_과정안내.docx` (12,551자, 2대 서버 구축 절차 포함) |
| 강의용 PPT | `고급실습_RecoveryCatalog.pptx` (38슬라이드, 노트 206~274자) |
| 운영 스크립트 | `운영스크립트/` 12종 + README (현업 반입용) |

**트랜스크립트 규격** — 장별 실습의 8단계 대신 고급용 7단계를 쓴다.
`[1] 실무 상황 / [2] 사전 설계 / [3] 환경 준비 / [4] 구축과 수행 / [5] 오류 주입과 대응 / [6] 검증 / [7] 운영 산출물과 확장 과제`
예제마다 오류 주입 2회 이상, 실제 발생하는 `ORA-`/`RMAN-` 코드 사용.

**검증** — `verify_adv.py` 18개 항목 전부 통과 (분량·템플릿·오류 코드·PPT 노트·스크립트 12종·`bash -n` 문법 검사 포함)
...
| **종합실습과제** | **완료** 과제지 15,854자 / 모범답안 46,327자 | — | — | — | **15과제 100점** | **자체점검 PASS** |
| **고급실습(RC)** | **완료** 과정안내 12,551자 | **완료** 38슬라이드 | — | — | **완료** 10개 예제 (평균 1,032줄) | **18/18 PASS** |
