...
| `rc_wallet_check.sh` | 실습 09 | TDE 지갑 상태 점검과 지갑 백업 | 매일 백업 전 |
| `rc_restore_validate.sh` | 실습 10 | 복구 가능성 검증 L1~L3 | 매일(L2)·주간(L3) |
| `dr_drill_runbook.md` | 실습 10 | 장애 대응·훈련 런북 | 상시 참조 |
| `rc_dashboard.sql` | 실습 10 | 백업·복구 체계 종합 현황 (설정 표준·성공률·아카이브 공백) | 주간 |
...
7. **훈련은 정리까지가 한 작업이다.** 복제본을 남기면 다음 훈련을 막고 운영 파일 시스템까지 압박한다. 정리에 `DROP DATABASE INCLUDING BACKUPS` 는 쓰지 않는다 — 복제하다 만 인스턴스에서는 원본의 백업을 지우려 든다.
