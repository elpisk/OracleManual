...
--           카탈로그가 VPD 모델(고급 실습 01)이면 EXEMPT ACCESS POLICY 도 있어야 한다
--           (없으면 오류 없이 0건으로 나온다)
--  주의   : 반드시 전체 DB를 볼 수 있는 계정으로 실행한다.
--           VPC 계정으로 실행하면 일부만 점검하고 정상으로 보고한다.
-- =============================================================================
SET LINESIZE 200 PAGESIZE 200 FEEDBACK OFF VERIFY OFF TRIMSPOOL ON
...
PROMPT ================================================================================
SET HEADING OFF
SELECT ' 생성 시각 : ' || TO_CHAR(SYSDATE,'YYYY-MM-DD HH24:MI:SS') ||
       '      카탈로그 : ' || SYS_CONTEXT('USERENV','DB_NAME') || ' @ ' ||
       SYS_CONTEXT('USERENV','SERVER_HOST') AS hdr
FROM   dual;
SET HEADING ON
...
SELECT d.name AS db_name,
  TO_CHAR(MAX(CASE WHEN s.incremental_level = 0 OR s.incremental_level IS NULL
              THEN s.completion_time END),'MM-DD HH24:MI')        AS last_full,
  TO_CHAR(MAX(CASE WHEN s.backup_type = 'L'
  ...
  CASE
    WHEN SYSDATE - MAX(CASE WHEN s.backup_type='L' THEN s.completion_time END)
...
PROMPT [ 섹션 2. 상세 ]
SELECT d.name AS db_name,
  ROUND(SYSDATE - MAX(CASE WHEN s.incremental_level = 0 OR s.incremental_level IS NULL
        THEN s.completion_time END),1)                             AS full_age_d,
  ROUND(SYSDATE - MAX(CASE WHEN s.backup_type='L'
  ...
  (SELECT COUNT(*) FROM rc_archived_log a
   ...
   AND    a.completion_time >
          (SELECT NVL(MAX(b.completion_time), SYSDATE-999)
           ...
           WHERE b.db_key = d.db_key AND b.backup_type='L'))           AS unbacked_arch,
...
SELECT msg FROM (
  ...
  FROM   rc_database d JOIN rc_backup_set s
         ON d.db_key = s.db_key AND s.backup_type = 'L'
  GROUP  BY d.name
  HAVING SYSDATE - MAX(s.completion_time) > &arch_warn_days
  UNION ALL
  SELECT '[WARN] ' || db_name || ' : ' ||
         DECODE(status, 'X', 'EXPIRED', 'U', 'UNAVAILABLE', status) ||
         ' 백업 조각 ' || COUNT(*) || '개'
  ...
  WHERE  NOT EXISTS (SELECT 1 FROM rc_rman_configuration c
                     ...
                     AND   c.name = 'RETENTION POLICY'
                     AND   c.value NOT LIKE '%NONE%')
);

PROMPT
PROMPT [ 섹션 4. 최근 7일 추세 (DB FULL / DB INCR) ]
...
WHERE  start_time > SYSDATE - 7 AND input_type IN ('DB FULL', 'DB INCR')
...
SELECT d.name AS db_name,
       NVL(MAX(CASE WHEN c.name = 'RETENTION POLICY' THEN c.value END),
           '*** 정책 없음 ***') AS retention
